...
import { salesReport } from "@/lib/reports.functions";
...
function AdminReports() {
  ...
  const runFn = useServerFn(salesReport);

  const { data, isFetching, refetch } = useQuery({
    queryKey: ["sales_report", from, to],
    queryFn: () => runFn({ data: { from: new Date(from).toISOString(), to: new Date(`${to}T23:59:59`).toISOString() } }),
  });
  ...
  const maxRev = Math.max(1, ...(data?.byDay.map((d) => d.revenueCents) ?? [1]));
